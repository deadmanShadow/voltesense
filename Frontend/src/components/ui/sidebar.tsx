/**
 * Sidebar — shadcn/ui collapsible sidebar block.
 *
 * Wraps Radix's `Dialog` (for the mobile Sheet view) and ships a context
 * provider that exposes the open/collapsed state to descendants. Used by
 * `AppShell` (top-level provider) and `AppSidebar` (the actual nav).
 *
 * MUST — changesFrontend.md §4 (Phase 3: layout migration). Mirrors the
 * official shadcn "sidebar" component verbatim so future block updates
 * drop in cleanly.
 */
...
>(function SidebarProvider(
  ...
  // isMobile kept as a plain boolean — we don't actually run a resize
  // listener today (VoltSense is desktop-first per PRD §29). The flag is
  // still exposed so descendants can opt into mobile-only behaviour later.
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist collapsed/expanded state in a cookie for next session.
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    },
    [setOpenProp, open],
  );

  // Helper to open sidebar from keyboard shortcut.
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "[[data-collapsible=offcanvas]_&]:translate-x-0",
        "[[data-collapsible=icon]_&]:translate-x-2 group-data-[collapsible=icon]:translate-x-0",
        className,
      )}
      {...props}
    />
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:hidden",
        showOnHover &&
          "group-focus/menu-item:opacity-100 group-hover/menu-item:opacity-100 data-[state=open]:opacity-100 peer-data-[active=true]/menu-button:text-sidebar-accent-foreground md:opacity-0",
        className,
      )}
      {...props}
    />
